...
- **Gravity**: after every placement, tokens fall straight down within
  their own column to fill empty space below them. Columns never affect
  each other -- if you place something in one column, nothing shifts in
...
- **Clearing**: whenever 10 or more tokens of the *same color* are
  ...
  resettles the board, that clears too, and so on (a cascade).
- **You win only when the entire board is empty.** There's no score, no
  target percentage, nothing else.
...
- **Undo** steps back through your whole move history, one move at a
  time. **Restart** resets the puzzle to its starting position.
...
- `config.js` -- everything that defines *this* puzzle: colors, the
  clear threshold, the board shape, the starting tokens, and the
  predetermined piece sequence (plus a documented, verified solution).
  Tweak numbers here to experiment with the rules.
...
- One non-obvious design fix worth knowing about: the win condition
  ("board completely empty") is checked after *every* move, and this
  puzzle is built around clearing color groups in waves. Early on, a wave
...
- Did groups of 10 feel like the right size to aim for -- too easy, too
  hard, or about right?
- Did clearing the whole board feel satisfying?
- When you lost, did it feel like your mistake, or like the game's fault?
- Did the board ever feel too crowded or cramped?
- Did the falling/shifting board make placement more interesting, or just
  confusing?
...
There's exactly one hand-built level right now, so "did I actually win"
is itself a useful data point -- if you get stuck, Undo is there to help
you rethink recent moves, and the full solution is documented in
`config.js` if you want to compare your approach to a known-working one.
